...
class FlushBot(Bot):
    def __init__(
        ...
        self.steps_done = 0
        self.metrics_file = (
            f"flush_bot_metrics_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
    ...
    def _select_cards_from_hand(self, G):
        try:
            ...
            if not hasattr(G, "hand") or not G.hand:
            ...
            sorted_values = sorted(
                value_counts.items(), key=lambda x: (len(x[1]), x[0]), reverse=True
            )
            ...
            if (
                len(sorted_values) >= 2
                and len(sorted_values[0][1]) >= 3
                and len(sorted_values[1][1]) >= 2
            ):
    ...
    def select_shop_action(self, G):
        ...
        if hasattr(G, "jokers") and len(G.jokers) >= 5:
        ...
        if hasattr(G, "shop"):
            ...
            for i, card in enumerate(shop_jokers):
                if (
                    ...
                    and card.get("label") not in attempted_purchases
                    and len(getattr(G, "jokers", [])) < 5
                ):
                    logging.info(
                        f"Attempting to buy specific Joker: {card.get('label')}"
                    )
                    attempted_purchases.add(card.get("label"))
    ...
    def _log_metrics(
        self, G=None, action=None, joker_purchased=0, shop_rerolled=0, joker_sold=0
    ):
        if G and action:
            ...
            flush_played = int(
                action[0] == Actions.PLAY_HAND and most_common_suit_count >= 5
            )
            cards_discarded = len(action[1]) if action[0] == Actions.DISCARD_HAND else 0
            action_type = action[0]
        else:
            flush_played = cards_discarded = most_common_suit = (
                most_common_suit_count
            ) = ""
            action_type = ""
    ...
    def sell_jokers(self, G):
        ...
        if len(G.jokers) > 4:
            for i, joker in enumerate(getattr(G, "jokers", []), start=1):
                if joker.name not in jokers_to_keep and len(G.jokers) > 4:
                    logging.info(f"Selling joker: {joker.name} at position {i}")
...
def run_bot():
    bot = FlushBot(
        deck="Plasma Deck", stake=1, seed=None, challenge=None, bot_port=12348
    )
